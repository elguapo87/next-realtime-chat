...
const RightSidebar = ({ selectedUser, setSelectedUser, showRightSide, setShowRightSide }: HomePageProps) => {
  ...
  return selectedUser && (
    <div className={`bg-[#8185b2]/10 text-white w-full relative overflow-y-scroll max-md:hidden ${showRightSide && "block!"}`}>
      ...
      <Image onClick={() => setShowRightSide && setShowRightSide(false)} src={assets.arrow_icon} alt="" className="absolute top-3 left-3 md:hidden max-w-7" />

      <div className="pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto">
        <Image src={selectedUser?.profilePic || assets.avatar_icon} alt=""  className="w-20 aspect-[1/1] rounded-full" />
      ...
      <div className="px-5 text-xs">
        ...
        <div className="mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80">
          {imagesDummyData.map((img, index) => {
            ...
            return (
              <div onClick={() => window.open(imageUrl)} key={index} className="cursor-pointer rounded">
                <Image src={imageUrl} alt="" width={100} height={100} className="w-30 h-full rounded-md" />
              </div>
            );
          })}
        </div>
